...
pub fn generic_image_manager<Env: ?Sized + drawer::HasImageManager, Store: drawer::ImageStore>(
    env: &Env,
) -> impl drawer::ImageManager<Store = Store> + Copy
where
    Env::ImageManager: drawer::ImageManager<Store = Store>,
{
    env.image_manager()
}
